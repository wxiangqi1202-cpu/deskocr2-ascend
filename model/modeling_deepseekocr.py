...
class DeepseekOCRConfig(DeepseekV2Config):
    ...
    def __init__(self, **kwargs):
        # Override MLA-related defaults to standard MHA values
        # DeepSeek-OCR uses standard MHA, not MLA
        kwargs.setdefault('kv_lora_rank', None)
        kwargs.setdefault('q_lora_rank', None)
        kwargs.setdefault('qk_rope_head_dim', 0)
        kwargs.setdefault('qk_nope_head_dim', 0)
        kwargs.setdefault('v_head_dim', 0)
        kwargs.setdefault('use_mla', False)
        ...
        if sam_model is not None and (input_ids.shape[1] != 1 or self.training) and torch.sum(images[0][1]).item() != 0:
            ...
            for image, crop_shape in zip(images, images_spatial_crop):
                ...
                if images_in_this_batch:
                    ...
                    inputs_embeds[idx].masked_scatter_(images_seq_mask[idx].unsqueeze(-1).to(next(self.parameters()).device), images_in_this_batch)
...
class DeepseekOCRForCausalLM(DeepseekV2ForCausalLM):
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, Cache):
                ...
                # get_max_length was removed in transformers >= 4.47, use get_max_cache_shape or None
    ...
    def infer(self, tokenizer, prompt='', image_file='', output_path = '', base_size=1024, image_size=640, crop_mode=True, test_compress=False, save_results=False, eval_mode=False):
        ...
        if not eval_mode:
            ...
            with torch.autocast(next(self.parameters()).device.type, dtype=torch.bfloat16):
                with torch.no_grad():
                    output_ids = self.generate(
                        input_ids.unsqueeze(0).to(next(self.parameters()).device),
                        images=[(images_crop.to(next(self.parameters()).device), images_ori.to(next(self.parameters()).device))],
                        images_seq_mask = images_seq_mask.unsqueeze(0).to(next(self.parameters()).device),
                        images_spatial_crop = images_spatial_crop,
                        # do_sample=False,
                        # num_beams = 1,
                        temperature=0.0,
        ...
        if '<image>' in conversation[0]['content'] and eval_mode:
                outputs = tokenizer.decode(output_ids[0, input_ids.unsqueeze(0).to(next(self.parameters()).device).shape[1]:])
